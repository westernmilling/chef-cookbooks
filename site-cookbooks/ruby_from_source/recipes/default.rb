...
remote_file "ruby-source" do
  ...
  path '/tmp/ruby-2.2.1.tar.gz'
  action :create
end

# tar_extract 'http://cache.ruby-lang.org/pub/ruby/2.1/ruby-2.2.1.tar.gz' do
tar_extract '/tmp/ruby-2.2.1.tar.gz' do
  action :extract_local
  target_dir '/usr/src'
  creates '/usr/src/ruby-2.2.1'
end

execute './configure CONFIGURE_OPTS=-fPIC' do
  action :run
  cwd '/usr/src/ruby-2.2.1'
  creates '/usr/src/ruby-2.2.1/config.status'
end

execute 'make' do
  action :run
  cwd '/usr/src/ruby-2.2.1'
  creates '/usr/src/ruby-2.2.1/ruby'
end
